...
enum WingAddress {
    ...
    /// Physical *input* connector scribble name, e.g. `/io/in/LCL/1/name`.
    /// Hardware-confirmed for the 24 LOCAL inputs on WING Rack firmware 3.1.
    ...
    static func allQueryAddresses() -> [String] {
        ...
        // Hardware-confirmed physical input names for the LOCAL bank. The WING
        // output nodes expose only `grp` and `in`, so output labels are app data.
